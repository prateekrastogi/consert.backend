...
const recommenderRqs = require('./recommendation-requests')
...
exports.listItems = function listItems (params) {
  const listItemsObservable = clientSendAsObservable(new recombeeRqs.ListItems(params)).pipe(
    map(items => R.map(convertRecommenderResponseItemToCommonMediaItem, items)),
    retry(RETRY_COUNT)
  )
...
function convertRecommenderResponseItemToCommonMediaItem (recombeeResponseItem) {

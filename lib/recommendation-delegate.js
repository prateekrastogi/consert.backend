...
exports.getRecommmendations = function getRecommmendations (context) {
  const {userId, count, itemId, route} = context
  ...
  const countryContext = of(context).filter(({route}) => route === '/country')

  const electronicContext = of(context).filter(({route}) => route === '/electronic')

  const jazzBluesClassicalContext = of(context).filter(({route}) => route === '/jazz-blues-classical')
...
function homePageRecommender (context) {
  const {userId, count, itemId} = context

  const homePageContext = of(context).filter(({route}) => route === '/')

  homePageContext.concatMap(context => {
    clientSendAsObservable(recommenderRqs.homePage(context))
  })
}
